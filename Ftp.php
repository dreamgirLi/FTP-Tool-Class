...
class Ftp
{
   ...
   {
      $this->host = \PhalApi\DI()->config->get('config.FTPhost');
      $this->user = \PhalApi\DI()->config->get('config.FTPuser');
      $this->pass = \PhalApi\DI()->config->get('config.FTPpwd');
      $this->port = \PhalApi\DI()->config->get('config.FTPport');
   }
    ...
    */
    public function upload($local_file='',$remote_file='', $permissions=NULL)
    { 
   ...
   {
      $result = ftp_get($this->conn, $local_file, $remote_file, FTP_BINARY);
      if ($result === FALSE){
            return FALSE;
      }
      return TRUE;
